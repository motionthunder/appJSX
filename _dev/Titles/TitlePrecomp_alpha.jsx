...
for (var i = 0; i < textLayers.length; i++) {
    ...
    layer.copyToComp(newComp);
    var newLayerInComp = newComp.layer(1);
    var newAnchor = newLayerInComp.property("Anchor Point").value;
    
    var anchorOffset = [originalAnchor[0] - newAnchor[0], originalAnchor[1] - newAnchor[1]];
    var layerCenter = [rect.left + rect.width / 2, rect.top + rect.height / 2];
    var anchorOffsetFromCenter = [layerCenter[0] - originalAnchor[0], layerCenter[1] - originalAnchor[1]];
    
    for (var t = newLayerInComp.inPoint; t <= newLayerInComp.outPoint; t += comp.frameDuration) {
        var originalPosAtTime = layer.position.valueAtTime(t, false);
        newLayerInComp.position.setValueAtTime(t, [originalPosAtTime[0] - minX + anchorOffset[0] - anchorOffsetFromCenter[0], originalPosAtTime[1] - minY + anchorOffset[1] - anchorOffsetFromCenter[1]]);
    }
